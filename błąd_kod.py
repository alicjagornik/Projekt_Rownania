...
Ania = BodyWeight(t=365, m=80, h=170, a=15, c=1500, f=1.3, sex="k")
...
kroki=[] #kolejne kroki czasowe powiekszone i-razy
...
for i in (1, 3000):
    ...
    if Ania.final_weight_analytical(365) >= Ania.final_weight_euler(h):
        euler.append(Ania.final_weight_analytical(365) - Ania.final_weight_euler(h) )
    else: euler.append(- Ania.final_weight_analytical(365) + Ania.final_weight_euler(h) )
    if Ania.final_weight_analytical(365) >= Ania.final_weight_runge_kutta(h):
        r_k.append(Ania.final_weight_analytical(365) - Ania.final_weight_runge_kutta(h))
    else: r_k.append(Ania.final_weight_runge_kutta(h) - Ania.final_weight_analytical(365))

plt.loglog(kroki, euler)
plt.loglog(kroki, r_k)
